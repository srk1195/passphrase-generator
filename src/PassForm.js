...
function PassForm() {
    ...
    const handleSubmit = (event) => {
        ...
        if (values.specialChars.length > 0 && values.minLength <= 100) {
            ...
            setValues({ ...values, generatedPassword: pwd, pwdLen: len, error: false, crackTime });
        } else {
        ...
        <div>
            <div className="PwdLength">
                {values.pwdLen === 0 ? (
                    ...
                    <h5>
                        Your Password is of <span className="crackTime">{values.pwdLen}</span> Charecter Length and can
                        take <span className="crackTime">{values.crackTime}</span> to crack
                    </h5>
                )}
            </div>
            <div>
                <form className="form-parent">
                    ...
                    <div className="form-child numWordsField">
                        <label htmlFor="numWords">
                            Number of Words
                            <select name="numWords" id="numWords" onChange={handleChange} value={values.numWords}>
                    ...
                    <div className="form-child minLengthField">
                        <label htmlFor="minLength">
                            ...
                            {values.minLength > 100 && (
                                <>
                                    <br />
                                    <span className="error"> Length Cannot be more than 100</span>{' '}
                                </>
                            )}
                        </label>
                    </div>
                    ...
                    <div className="form-child specialCharsField">
                        <label htmlFor="specialChars">
                            ...
                            {values.specialChars.length === 0 && (
                                <>
                                    <br />
                                    <span className="error"> You should enter atleast 1 Charecter</span>{' '}
                                </>
                            )}
                        </label>
                    </div>
